    ...
    try:
        ...
        if not manual_supplier:
            manual_supplier = Supplier(
                email='manual@system.local',
                password_hash='manual_system_account',
                tax_id="MANUAL000000",
                company_name="手动采购",
                contact_name='系统',
        ...
        # 获取PR号用于生成PO编号
        pr = rfq.pr if hasattr(rfq, 'pr') else None
        pr_number = pr.pr_number if pr else f"PR-{rfq_id}"
        ...
        for quote_data in quotes_data:
            ...
            if not existing_po:
                # 生成PO编号（手动报价可能创建多个PO，使用计数器）
                po_count = len(created_pos) + 1
                if len(quotes_data) == 1:
                    po_number_gen = pr_number
                else:
                    po_number_gen = f"{pr_number}-{chr(64 + po_count)}"

                po = PurchaseOrder(
                    po_number=po_number_gen,
                    rfq_id=rfq_id,
                    quote_id=quote.id,
                    supplier_id=manual_supplier.id,
                    supplier_name=supplier_name,
                    ...
                    status='pending_admin_confirmation',  # 需要管理员确认
                    confirmed_at=None,
...
@bp.route('/<int:rfq_id>/fix-pending', methods=['POST', 'OPTIONS'])
def fix_pending_tasks(rfq_id):
    """修复待处理的通知任务（将pending任务标记为sent）"""
    if request.method == 'OPTIONS':
        return "", 204
    ...
    try:
        ...
        # 查找所有pending任务
        pending_tasks = RFQNotificationTask.query.filter_by(
            rfq_id=rfq_id,
            status='pending'
        ).all()
        ...
        return jsonify({
            "message": f"已修复 {fixed_count} 个待处理任务",
    ...
    try:
        ...
        if rfq.classification_task_id:
            try:
                ...
                logger.warning(f"无法获取Celery任务状态: {e}")
...
def create_po_from_rfq(rfq_id):
    ...
    try:
        ...
        # 权限检查：只有主管、管理员和超管可以选标
        user_role = request.headers.get('User-Role', '')
        if user_role not in ['supervisor', 'admin', 'super_admin']:
            return jsonify({"error": "权限不足，只有主管及以上角色可以进行选标"}), 403
        ...
        for quote in quotes:
            ...
            supplier_quotes[sid]['quotes'].append(quote)
            supplier_quotes[sid]['total_price'] += float(quote.total_price or 0)
            supplier_quotes[sid]['lead_time'] = max(supplier_quotes[sid]['lead_time'], quote.lead_time or 0)
            supplier_quotes[sid]['items'].append({
                'item_name': quote.item_name,
                'quantity': quote.quantity_requested,
                'unit': quote.unit,
                'unit_price': float(quote.total_price or 0) / max(quote.quantity_requested or 1, 1),
                'subtotal': float(quote.total_price or 0)
            })
        ...
        for supplier_id, data in supplier_quotes.items():
            # 使用PR编号作为PO编号
            if len(supplier_quotes) == 1:
                po_number = pr_number
            else:
                supplier_count += 1
                po_number = f"{pr_number}-{chr(64 + supplier_count)}"
            ...
            po = PurchaseOrder(
                ...
                quote_data=json.dumps({'items': data['items'], 'quote_ids': [q.id for q in data['quotes']]}),
...
def delete_rfq(rfq_id):
    ...
    try:
        ...
        # 3. 删除RFQ（级联删除相关items, quotes, tasks等）
        ...
        db.session.rollback()
        traceback.print_exc()
        return jsonify({"error": f"删除RFQ失败: {str(e)}"}), 500
