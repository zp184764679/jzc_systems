...
wework_oauth_bp = Blueprint('wework_oauth', __name__)
...
@wework_oauth_bp.route('/authorize', methods=['GET'])
def authorize():
    """
    生成企业微信授权URL
    ...
    try:
        ...
        logger.info(f"生成企业微信授权URL: {full_auth_url}")
...
@wework_oauth_bp.route('/callback', methods=['GET'])
def callback():
    """
    企业微信授权回调
...
@wework_oauth_bp.route('/confirm', methods=['POST'])
...
@wework_oauth_bp.route('/bind', methods=['POST'])
def bind_wework():
    """
    已有账号绑定企业微信
