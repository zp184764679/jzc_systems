...
- [x] 邮件集成 - 智能任务创建
  ...
  - [x] 集成到 TaskFormModal
- [x] 文件中心 (File Hub)
  - [x] 统一文件索引表 (`file_index`)
  - [x] 多维度查询（按订单/项目/供应商/客户）
  - [x] 文件分类管理（采购式样书、图纸、合同、发票等）
  - [x] 文件上传（支持关联业务实体）
  - [x] 批量下载（ZIP 打包）
  - [x] 统计卡片（总文件数、本月上传）
  - [x] 响应式布局（移动端支持）
...
- [ ] **子系统集成** - 在采购、报价系统上传文件时自动同步到文件中心索引
  - 修改 `采购/backend/routes/invoice_routes.py` - 发票上传同步
  - 修改 `报价/backend/api/drawings.py` - 图纸上传同步
  - 修改 `Portal/backend/routes/files.py` - 项目文件同步
- [ ] **历史数据迁移** - 将现有文件迁移到统一索引
  - 创建迁移脚本 `Portal/backend/scripts/migrate_to_file_index.py`
  - 迁移 Portal 项目文件
  - 迁移采购发票
  - 迁移报价图纸
- [ ] **权限控制增强** - 按角色/项目权限过滤文件访问
  - 普通用户只能查看自己上传或参与项目的文件
  - 主管可查看部门内所有文件
  - 管理员可查看全部文件
