// src/pages/inventory/DeliveryCheck.jsx
// 交货核销（简化版）
...
export default function DeliveryCheck() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err)
      alert('核销失败: ' + (err.response?.data?.error || err.message))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div style={L.page}>
      <div style={L.header}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>交货核销</h2>
      ...
      <div style={{ ...L.card, marginBottom: 12 }}>
        <div style={L.grid3}>
          ...
          <div style={L.fieldCol}>
            ...
            <input style={L.input} value={form.bin_code} onChange={(e) => onChange('bin_code', e.target.value)} placeholder="可选" />
          </div>
          <div style={L.fieldCol}>
